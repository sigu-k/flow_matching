#!/usr/bin/env python3
"""
Phase 1 Post-hoc: CIFAR-10 Flow Matching with configurable timestep sampler.

Copy of train_phase1_static.py that ADDITIONALLY dumps fp16, weights-only
snapshots at a fixed epoch interval so any EMA decay can be reconstructed
offline afterwards (reconstruct_ema.py). All outputs (resume checkpoints AND
snapshots) go to a SEPARATE directory tree: checkpoints/phase1_posthoc/, so the
original phase1_static runs are never touched.

Run from: flow_matching/examples/image/
  python train_phase1_posthoc.py --config configs/phase1_static/ln_mu-0.8.yaml
  python train_phase1_posthoc.py --config configs/phase1_static/ln_mu-0.8.yaml --snapshot-every 2
  python train_phase1_posthoc.py --config configs/phase1_static/ln_mu-0.8.yaml --dry-run
"""
...
WANDB_PROJECT = "phase1-posthoc"
_REPO_ROOT = Path(__file__).resolve().parents[2]
# Separate tree from phase1_static so original runs are never touched.
